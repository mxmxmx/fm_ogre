...
#define RESOLUTIONSWITCH 0x0 // PORTBbits.RB4
#define SAMPLESWITCH 0x0 // PORTAbits.RA4
...
#define cvpitch (cvdata[3])
#define cvfb (cvdata[1])
#define cvfm (cvdata[2])
#define cvpm (cvdata[0])
#define cvpmknob (cvdata[6])
#define cvfmknob (cvdata[5])
#define cvfbknob (cvdata[7])
#define cvpitchknob (cvdata[4])
#define ADC_FIRST (0)
#define ADC_LAST (7)
...
//   Pins RB6 and RB7 are the LED ports, RB5 is unused
    ...
    DAC1CONbits.DACFDIV = 8;  // (was 3, then 6) divide Fosc to drive 
...
#define BASELINE_CVPITCHINCR
    ...
    curpitchval = cvpitchknob + (2047 - cvpitch) ;
    //   we don't have the LFO switch ... 
    curpitchincr = 
        (exp_table [
          (curpitchval < 0) ? 0 :
...
#define BASELINE_CURFREQMOD
    ...
    curfreqmod = cvfmknob * (((long)2047) - cvfm) << 5;
